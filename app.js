...
const bot = require('./controllers/new_chat.js');
...
// Table storage
var tableName = "NumerikaTable"; // You define
var storageName = "numerikastorage1"; // Obtain from Azure Portal
var storageKey = "78xc+nEaFU/G7NarRmb8gra+OONXfVEWqFYQQVNjcQWqZh3b1QKqDQeT/z+9wrCsSnkiHa81B1xgqVac0SPj9Q=="; // Obtain from Azure Portal
...
// The following message will show on the console onde you run 'node example.js'
// Opening localhost:3000 in the browser will give an error as expected
app.listen(process.env.PORT || 3000, () => {
  console.log('Numerika listening on port 3000!');
});
...
function init(app){
  ...
  */
  const connector = new builder.ChatConnector({
      appId: 'b8b70c07-8acb-4c1c-bdf7-5d08144d5a10', //should be env.BOT_APP_ID,
      appPassword: 'tiswbqHPFO58222({+cZKL{', //should be env.BOT_APP_PASSWORD,
  });
  ...
  var luisAppId = process.env.LuisAppId || 'ae9707a6-c99a-4c05-9543-a7d6fde20876';
  var luisAPIKey = process.env.LuisAPIKey || 'b747acba34ac456d8b4bc7f9fcfcb646';
  ...
  bot.dialog('WorkHappy',
      function(session, args){
          let workHappyText = 'We are a people-oriented company, we really care about our employees, as we believe they are a key ingredient to our company success';
  ...
  bot.dialog('Positions',
      function(session, args){
          let positionsText = 'We have several open positions at this time!';
  ...
  bot.dialog('Turnover',
      function(session, args){
          let turnoverText = 'Our turnover rate is one of the lowest, as you can see from our employees’ feedback here: LINK. I\'ve been in the company myself for many years.';
  ...
  function addHistory(session){
      ...
      //session.conversationData[timestamp.toISOString().replace
